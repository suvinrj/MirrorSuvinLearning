...
{
    Randomize(); // ✅ Call Randomize manually when the player clicks a button
}
        ...
        void Randomize()
{
    ...
    // disable all previously enabled objects before clearing
    foreach (GameObject g in enabledObjects)
    {
        g.SetActive(false);
    }
    ...
    // **Randomizing Gender**
    gender = GetPercent(50) ? Gender.Male : Gender.Female;

    // **Randomizing Race**
    race = GetPercent(70) ? Race.Human : Race.Elf;

    // **Randomizing Head Covering**
    int headCoveringRoll = Random.Range(0, 3);
    headCovering = (HeadCovering)headCoveringRoll;

    // **Randomizing Skin Color (No Color Application)**
    if (race == Race.Human)
    {
        int colorRoll = Random.Range(0, 3);
        skinColor = (SkinColor)colorRoll;
    }
    else
    {
    ...
    Debug.Log($"✅ Character Randomized - Gender: {gender}, Race: {race}, HeadCovering: {headCovering}");
}
